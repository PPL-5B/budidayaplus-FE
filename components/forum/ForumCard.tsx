...
const ForumCard: React.FC<ForumCardProps> = ({ forum, onDeleteSuccess, onVoteSuccess, onUpdateSuccess }) => {
  ...
  const [upvotes, setUpvotes] = useState<number>(forum.upvotes);
  ...
  // Ambil data vote saat pertama render
  useEffect(() => {
    const fetchVotes = async () => {
      try {
        const data = await fetchVoteSummary(forum.id);
        setUpvotes(data.upvotes);
        setUserVote(data.user_vote === 'up' ? 'upvote' : null);
      } catch (err) {
        console.error('Gagal mengambil vote summary:', err);
      }
    };
    fetchVotes();
  }, [forum.id]);
    ...
    <div className="relative w-full max-w-[338px] bg-white rounded-[10px] shadow-md p-4 transition-all duration-200 overflow-hidden">
      ...
      <div className={cn('mt-2', isOwner ? 'w-[166px]' : 'w-[140px]')}>
        <ForumCardFooter
          ...
          isEditing={isEditing}
          upvotes={upvotes}
          userVote={userVote === 'upvote' ? 'upvote' : null}
