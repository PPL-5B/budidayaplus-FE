...
import { columns } from '@/components/fish-death'; // ganti sesuai path aslimu
import { FishDeath } from '@/types/fish-death';
import User from '@/types/auth/user';
import { format } from 'date-fns';
import { id } from 'date-fns/locale';

// ✅ Mock user sesuai tipe `User`
const mockUser: User = {
  id: 99,
  first_name: 'Udin',
  last_name: 'Sedunia',
...
// ✅ Mock FishDeath (tanpa cause karena gak dipakai di tabel)
...
describe('FishDeathTableColumns', () => {
  ...
  test('all column headers include icons', () => {
    columns.forEach((col) => {
      ...
      expect(container.querySelector('svg')).toBeInTheDocument(); // Lucide icon
    });
  });
});
